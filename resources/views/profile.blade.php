@extends('layouts.app')
@section('title',$profile->name.' — GreenPoint')
@section('content')
...
<div class="two-col profile-cols">
 <section>
  <div class="card"><h2>Badge ที่ได้รับ</h2><div class="badge-row">
   @forelse($badges as $b)
   <div class="badge"><i>{{$b->icon}}</i><b>{{$b->name}}</b><small>{{$b->description}}</small><small>ได้รับเมื่อ {{\Illuminate\Support\Carbon::parse($b->pivot->earned_at)->format('d/m/Y')}}</small></div>
   @empty<p class="empty">ยังไม่มี Badge</p>@endforelse
  </div></div>
  <h2 class="section-title">โพสต์ของ {{$profile->name}}</h2>
  @forelse($posts as $p)
